...
def load_lexicon():
    lexicon = {}
    fin = open('lexicon/vader_lexicon.txt', 'r', encoding='utf-8')
    lines = fin.readlines()
    fin.close()
...
def load_glove(vocab):
    ...
    fout = open('glove/glove.lexicon.txt', 'w', encoding='utf-8')
    lines = fin.readlines()
    for line in tqdm(lines):
...
def calculate_distance():
    fin = open('glove/glove.lexicon.txt', 'r', encoding='utf-8')
    ...
    for i in tqdm(range(0, len(vocab))):
        ...
        for j in range(i + 1, len(vocab)):
            matrix[i][j] = np.linalg.norm(
                embedding[vocab[i]] - embedding[vocab[j]])
            matrix[j][i] = np.linalg.norm(
                embedding[vocab[i]] - embedding[vocab[j]])
    data['words'] = vocab
    data['dis'] = matrix
...
def find_top_k():
    ...
    for i in tqdm(range(len(new_vocab))):
        ...
        knn_dict[new_vocab[i]] = sorted(
            dis_dict.items(), key=lambda x: x[1], reverse=False)[1:11]
...
def refine(alpha, beta):
    ...
    fin1 = open('glove/glove.lexicon.txt', 'r', encoding='utf-8')
    ...
    if not cond1 or not cond2:
        ...
        W = np.zeros((len(vocab), len(vocab))).astype(
            'float32')  # (35739,35739)
        S = np.zeros((len(vocab), len(vocab))).astype(
            'float32')  # (35739,35739)
    ...
    while True:
        ...
        B = alpha + beta * \
            np.matmul(W, np.transpose(np.ones(len(vocab))))  # (35739,1)
        V = M / B  # (300,35739)
        ...
        last = np.copy(v)
        now = np.copy(V)
        move = np.sum(np.sum(now.T - last.T, axis=1) /
                      300, axis=0) / len(vocab)

        # if epoch % 10 == 0:
        print('epoch:{} loss:{} move:{}'.format(
            epoch, round(loss, 6), abs(move)))
        v = V
        # if last_loss - loss < 1:
        if loss < 1:
            new_vec = v.T.tolist()
...
def eval(word):
    ...
    old = tsne.fit_transform([old_embedding[i[0]]
                             for i in old_top_k] + [old_embedding[word]])
    new = tsne.fit_transform([new_embedding[i[0]]
                             for i in new_top_k] + [new_embedding[word]])
